...
class MultiTPLoader(Dataset):
    def __init__(self, dataset):
        ...
        for volume_paths, clin_vars, dx in dataset:
            ...
            npy_paths = [path.replace('/media/rohan/ThirdHardDrive/Combined_FSL', '/home/rohan/Documents/Alzheimers/embeddings').replace('.nii', '.npy') for path in volume_paths]
            ...
            self.embeddings.append((volumes.cuda(), torch.Tensor(clin_vars).cuda(), torch.LongTensor([dx]).cuda()))
...
class DataParser:
    def __init__(self, dataset_type, batch_size, path = '/media/rohan/ThirdHardDrive/Combined_FSL', splits = [0.8, 0.2]):
        random.seed(1)
        ...
        paths, df, type_dict = create_class_dict(path, clin_vars)
        ...
        data_types = [(self.create_3_classification, SingleTPLoader), (self.create_longitudinal, MultiTPLoader), (self.create_2_classification, SingleTPLoader)]
    ...
    def create_2_classification(self, df, type_dict, paths, clin_vars):
        return self.create_classification(df, type_dict, paths, clin_vars, dx_cap = 3)

    # creates the dataset for 3-way classification between AD/NC/MCI (based on dx_cap parameter)
    def create_classification(self, df, type_dict, paths, clin_vars, dx_cap = 2):
        gts = ['CN', 'Dementia', 'MCI']
        selected, data = [], []
    ...
    def create_longitudinal(self, df, type_dict, paths, clin_vars):
        ...
        for idx, (imids, dx) in enumerate(selected):
            ...
            result.append((volume_paths, data[3*idx:3*idx+3], dx))
    ...
    def create_dataset(self, dataset, splits, loader_class, in_batch_size):
        ...
        for split_ratio in splits:
            ...
            max_idx += chunk
            subsets.append(DataLoader(loader_class(dataset[min_idx:max_idx]), batch_size = in_batch_size, shuffle = True))
