...
class Constants:
    ...
    EMBEDDING_PATH = '/home/rohan/Documents/Embeddings/embeddings_288'
    SPLITS = [0.8, 0.2]
    LOAD_PATHS = True
...
def plot_heatmap(cam, guided_grads, orig_vols, label, thresh = 50):
    ...
    for i in range(3):  
        render(orig_vols[:, :, :, :], guided_grads, label, label.split(' ')[-1], i)
...
def main(c):
    ...
    model.embedding_net.requires_grad_()
    ...
    features = torch.zeros(3, 24, *shape_dict[idx])
    gradients = torch.zeros(3, 24, *shape_dict[idx])
    ...
    model.embedding_net.model[2 * idx].layers[0].register_full_backward_hook(save_grad)
    model.embedding_net.model[2 * idx].layers[0].register_forward_hook(save_conv)
    ...
    for idx, (volume_paths, clin_vars, dx) in enumerate(parser.full_testing_set()):
        ...
        neuron_importance = torch.mean(gradients, axis = (2, 3, 4)) # (3, 24)   
        cam = torch.zeros((3, *features.shape[2:]), dtype = torch.float32)
        for j in range(3):
            for i, ni_w in enumerate(neuron_importance[j]):
                cam[j, :, :, :] += ni_w * features[j, i, :, :, :]
